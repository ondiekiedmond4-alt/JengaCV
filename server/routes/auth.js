...
router.post('/register', async (req, res) => {
  try {
    ...
    const inserted = await pool.query(
      `INSERT INTO users (email, password_hash, name, marketing_opt_in, downloads_remaining)
       VALUES ($1, $2, $3, $4, 0) RETURNING id, name, downloads_remaining, marketing_opt_in`,
      [normalizedEmail, passwordHash, name.trim(), !!marketingOptIn]
    );
...
router.get('/me', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT name, email, downloads_remaining, marketing_opt_in FROM users WHERE id = $1',
      [req.userId]
    );
